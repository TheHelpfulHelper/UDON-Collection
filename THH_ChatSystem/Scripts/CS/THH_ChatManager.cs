...
public class THH_ChatManager : UdonSharpBehaviour
{
    ...
    public THH_ChatLog ChatLogManager;
    ...
    {
        ChatLogManager.Log($"<color=yellow>[System]: Player <color=lime>{player.displayName}</color> has joined.</color>");
    }
    public override void OnPlayerLeft(VRCPlayerApi player)
    {
        ChatLogManager.Log($"<color=yellow>[System]: Player <color=lime>{player.displayName}</color> has left.</color>");
    }
    ...
    public void Update()
    {
        ...
        if (messageSending)
        {
            ...
            {
                int messageID = messageCounter + 1;
        ...
        if (CHAT_MESSAGE != last_ChatMessage && !string.IsNullOrEmpty(CHAT_MESSAGE))
        {
            ...
            if (Int32.Parse(M[2]) > messageCounter)
            {
    ...
    bool ProcessMessage()
    {
        if (locked)
        {
            ChatLogManager.Log($"<i><color=red>You are sending messages too quickly</color></i>");
            return false;
        }
        ...
        else
        {
            if (autoRetry)
            {
                ...
                if (retryCount == maxRetries)
                {
                    ...
                    ChatLogManager.Log($"<i><color=red>Your message could not be delivered</color></i>");
                    autoRetry = false;
                    retryCount = 0;
                    SetInputFieldLocked(false);
                }
